...
from agent import run_agent as run_tool_agent  # reuses the full tool-calling loop
...
def classify_question(user_question):
    ...
    Classification only ever looks at the current question, not history.
    """
...
def handle_document(user_question, history=None):
    """
    Document path: retrieve the best matching chunk, then have the LLM
    answer using only that retrieved content (plus prior conversation turns).
    """
...
def handle_conversational(user_question, history=None):
    """
    Conversational path: answer directly, no tools, no retrieval,
    but still uses prior conversation turns for context.
    """
    messages = [
        {
            "role": "system",
...
def route(user_question, history=None):
    """
    Main router entry point: classifies the question, prints the routing
    decision, then dispatches to the correct handler, passing history through.
    """
    ...
    if label == "document":
        return handle_document(user_question, history)
    elif label == "tool":
        return run_tool_agent(user_question, history)  # full ReAct loop, history-aware
    else:
...
def main():
    load_collection()
    print("Router agent ready (Day 24 — with history support). Type 'exit' to quit.\n")
    ...
    while True:
        ...
        answer = route(question, history=history)
